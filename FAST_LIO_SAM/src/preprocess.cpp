...
void Preprocess::avia_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  pl_full.clear();
  double t1 = omp_get_wtime();
  double msg_time = static_cast<double>(msg->header.stamp.toNSec());
  ...
  pcl::PointCloud<livox_point_ros::Point> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);

  // points.resize(plsize);
  // memcpy(points.data(), msg->data.data(),
  //       plsize * sizeof(LivoxPointXyzrtlt));
// cout<<"plsie: "<<plsize<<endl;
  //清空缓存内的点云并预留足够空间
  for(int i=0; i<N_SCANS; i++)
  {
  ...
  if (feature_enabled)
  {
    ...
    for(uint i=1; i<plsize; i++)
    {
      if((pl_orig.points[i].line < N_SCANS) && ((pl_orig.points[i].tag & 0x30) == 0x10 || (pl_orig.points[i].tag & 0x30) == 0x00))
      {
        ...
        pl_full[i].curvature = (pl_orig.points[i].timestamp - msg_time) / float(1000000); //use curvature as time of each laser points
    ...
    for(uint i=1; i<plsize; i++)
    {
      if((pl_orig.points[i].line < N_SCANS) && ((pl_orig.points[i].tag & 0x30) == 0x10 || (pl_orig.points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          pl_full[i].z = pl_orig.points[i].z;
          pl_full[i].intensity = pl_orig.points[i].intensity;
          pl_full[i].curvature = (pl_orig.points[i].timestamp - msg_time) / float(1000000); // use curvature as time of each laser points, curvature unit: ms
